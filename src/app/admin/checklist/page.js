...
export default function ChecklistOnlinePage() {
  ...
  // Monta shareUrl só no client (evita mismatch)
  useEffect(() => {
  ...
  // (Opcional) rascunho local antes de criar online
  useEffect(() => {
    if (cid) return
    try {
  ...
  function addSection(t) {
    ...
    if (!title) return
    setData(prev => {
      const next = [...prev, { id: uid(), title, open: true, items: [] }]
      scheduleSave(next)
      return next
    })
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      {/* Sem styled-jsx => evita hash jsx-xxxx e mismatch */}
      <style>{`
        @media print {
