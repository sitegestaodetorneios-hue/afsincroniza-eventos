        ...
        {abaAtiva === 'dashboard' && (
          <div className="max-w-7xl mx-auto animate-in fade-in duration-500">
            <div className="flex justify-between items-end mb-8">
                ...
                <div className="bg-white px-6 py-3 rounded-2xl border border-slate-200 shadow-sm text-right"><span className="block text-[10px] font-bold uppercase text-slate-400 tracking-wider">Total Inscritos</span><span className="text-3xl font-black text-blue-600 leading-none">{equipes.length}</span></div>
            </div>
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                    ...
                    <tr><th className="p-6">Time / Local</th><th className="p-6">Responsável</th><th className="p-6">WhatsApp</th><th className="p-6 text-center">Pagamento</th><th className="p-6 text-center">Documento</th><th className="p-6 text-right">Ações</th></tr>
                    </thead>
                    ...
                    {!loading && equipes.map((eq) => (
                        <tr key={eq.id} className="hover:bg-blue-50/50 transition-colors group">
                            ...
                            <td className="p-6">{waLink(eq.whatsapp) ? <a href={waLink(eq.whatsapp)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-green-50 text-green-700 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-green-100 transition-colors border border-green-100 shadow-sm"><Phone size={14} /> {formatPhoneDisplay(eq.whatsapp)}</a> : <span className="text-slate-400 font-semibold flex items-center gap-2 text-xs"><AlertCircle size={14}/> Inválido</span>}</td>
                            ...
                            <td className="p-6 text-center">
                                {eq.termo_url ? (
                                    <div className="flex flex-col items-center gap-2">
                                        <a href={eq.termo_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase hover:bg-blue-100 border border-blue-200"><FileText size={14}/> Ver Termo</a>
                                        {!eq.termo_assinado ? (
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button onClick={() => gerenciarEquipe(eq.id, 'reject_doc')} className="text-red-300 hover:text-red-500 p-1"><Trash2 size={12}/></button>
                                            </div>
                                        )}
                                    </div>
                                ) : ( <span className="text-slate-300 text-xs font-bold uppercase tracking-wider flex flex-col items-center gap-1"><AlertCircle size={14}/> Pendente</span> )}
                            </td>
                            <td className="p-6 text-right">
                                <div className="flex justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                    {!eq.pago && <button onClick={() => gerenciarEquipe(eq.id, 'approve')} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 transition-all border border-blue-100" title="Aprovar Pagamento"><DollarSign size={18} /></button>}
                                    <button onClick={() => gerenciarEquipe(eq.id, 'delete')} className="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-600 transition-all border border-red-100" title="Excluir Equipe"><Trash2 size={18} /></button>
                                </div>
                            </td>
                        </tr>
                    ))}
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-200">
                    ...
                    <div className="space-y-4">
                        ...
                        <div><label className="text-[10px] font-black mb-1 block uppercase text-slate-400">Título</label><input className="w-full p-2 bg-slate-50 border rounded-lg font-bold" value={config.titulo_futsal || ''} onChange={(e) => setConfig({ ...config, titulo_futsal: e.target.value })} /></div>
                        <div><label className="text-[10px] font-black mb-1 block uppercase text-slate-400">Descrição</label><textarea className="w-full p-2 bg-slate-50 border rounded-lg font-medium h-20" value={config.desc_futsal || ''} onChange={(e) => setConfig({ ...config, desc_futsal: e.target.value })} /></div>
                ...
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-200">
                    ...
                    <div className="space-y-4">
                        ...
                        <div><label className="text-[10px] font-black mb-1 block uppercase text-slate-400">Título</label><input className="w-full p-2 bg-slate-50 border rounded-lg font-bold" value={config.titulo_society || ''} onChange={(e) => setConfig({ ...config, titulo_society: e.target.value })} /></div>
                        <div><label className="text-[10px] font-black mb-1 block uppercase text-slate-400">Descrição</label><textarea className="w-full p-2 bg-slate-50 border rounded-lg font-medium h-20" value={config.desc_society || ''} onChange={(e) => setConfig({ ...config, desc_society: e.target.value })} /></div>
                        <div className="grid grid-cols-2 gap-2">
                    ...
                    {listaPatrocinios.map(p => (
                        <div key={p.id} className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition-shadow group">
                            <div className="flex items-center gap-4">
                                <div className={`w-3 h-10 rounded-full ${p.cota === 'MASTER' ? 'bg-blue-600 shadow-[0_0_10px_rgba(37,99,235,0.4)]' : p.cota === 'RODAPE' ? 'bg-slate-800' : 'bg-green-500'}`}></div>
                                <div>
