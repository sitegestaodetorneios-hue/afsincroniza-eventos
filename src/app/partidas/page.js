...
import { ArrowLeft, Loader2, Calendar, Volume2, Clock, ChevronDown } from 'lucide-react'
...
function logoOf(team) {
  const u = team?.escudo_url || team?.logo_url
  ...
  async function loadData(id = '') {
    ...
    try {
      ...
      if (!targetId && json.etapa?.id) {
        setFiltroId(String(json.etapa.id))
      }
              ...
              <div key={titulo} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                {index === 1 && patrocinadoresCarrossel.length > 0 && (
                  <div className="mb-8 rounded-2xl overflow-hidden border border-slate-200 shadow-sm transition-all hover:shadow-md">
                    <a href={patrocinadoresCarrossel[indexCarrossel].link_destino || '#'} target="_blank" rel="noreferrer" className="relative block h-20 md:h-24">
                ...
                <div className="grid gap-4">
                  {listaJogos.map(j => {
                    const logoA = logoOf(j.equipeA)
                    const logoB = logoOf(j.equipeB)

                    return (
                      <div
                        key={j.id}
                        className={`bg-white p-5 rounded-2xl border transition-all shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 border-slate-200 ${
                          j.status === 'EM_ANDAMENTO' ? 'ring-2 ring-red-500 shadow-red-100' : 'hover:border-blue-300 hover:shadow-md'
                        }`}
                      >
                          ...
                          <div className="flex items-center justify-between w-full gap-2">
                            <span className="font-black text-slate-800 text-xs md:text-sm text-right flex-1 truncate uppercase tracking-tighter">
                              <span className="inline-flex items-center justify-end gap-2">
                                ...
                                {logoA ? (
                                  <img src={logoA} alt="" className="h-7 w-7 rounded-lg border border-slate-200 bg-white object-cover flex-shrink-0" loading="lazy" />
                                ) : (
                            ...
                            <span className="font-black text-slate-800 text-xs md:text-sm text-left flex-1 truncate uppercase tracking-tighter">
                              <span className="inline-flex items-center justify-start gap-2">
                                {logoB ? (
                                  <img src={logoB} alt="" className="h-7 w-7 rounded-lg border border-slate-200 bg-white object-cover flex-shrink-0" loading="lazy" />
                                ) : (
                                  <div className="h-7 w-7 rounded-lg bg-slate-100 border border-slate-200 flex-shrink-0" />
                                )}
                                <span className="truncate">
        ...
        <footer className="mt-20 pt-10 border-t border-slate-200">
          ...
          <div className="pt-8 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-500">
              ...
              <a href="https://wa.me/5547997037512" target="_blank" rel="noreferrer" className="text-blue-500 hover:text-blue-400 transition-colors flex items-center gap-1.5 group">
